...
function generateRepoPage(repoId, repoName, githubUrl) {
  const componentName = repoId.replace(/-/g, "_").split("_").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join("");
  
  return `import { View, Text, Pressable, ScrollView, Alert, StyleSheet } from "react-native";
