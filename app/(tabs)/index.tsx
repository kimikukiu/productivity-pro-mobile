...
// ============================================================
// RECENT COMMANDS (Terminal-style logs)
// ============================================================
function useRecentCommands() {
  const [commands, setCommands] = useState<{ ts: string; cmd: string; status: "success" | "error" | "running" }[]>([
    { ts: "09:28:15", cmd: "nmap -sV --open 192.168.1.0/24", status: "success" },
    { ts: "09:29:42", cmd: "sqlmap -u target.com --dbs --batch", status: "success" },
    { ts: "09:31:10", cmd: "nikto -h https://target.com -maxtime 90", status: "running" },
    { ts: "09:32:55", cmd: "hydra -l admin -P passlist.txt ssh://10.0.0.5", status: "error" },
    { ts: "09:34:20", cmd: "gobuster dir -u target.com -w wordlist.txt", status: "success" },
  ]);
  ...
  useEffect(() => {
    const cmds = [
      ...
      "wpscan --url target.com --enumerate u,t,p",
      "metasploit use exploit/multi/handler",
      "aircrack-ng -w passlist.txt capture.cap",
      "john --wordlist=rockyou.txt hash.txt",
      "tcpdump -i wlan0 port 443 -w capture.pcap",
      "netcat -lvnp 4444 -e /bin/bash",
      "chmod +x exploit.sh && ./exploit.sh",
      "curl -X POST -d 'user=admin&pass=test' target.com/login",
    ];
    const statuses: Array<"success" | "error" | "running"> = ["success", "error", "running", "success", "success"];
...
export default function ControlCenterScreen() {
  ...
  const commandsRef = useRef<ScrollView>(null);

  // Import all modules count from context
  const totalModules = ALL_MODULES.length;
  const runningModules = ALL_MODULES.filter((m: any) => m.status === "RUNNING").length;
  const totalTools = ALL_TOOLS.length;
  const runningTools = ALL_TOOLS.filter((t: any) => t.status === "RUNNING").length;
  ...
  const getCmdStatusColor = (status: string) => {
    const map: Record<string, string> = { success: "#00ff88", error: "#ff3b5c", running: "#ffff00" };
  ...
  useEffect(() => {
    commandsRef.current?.scrollToEnd({ animated: true });
  }, [commands]);
    ...
    <ScreenContainer className="bg-background" edges={["top", "left", "right"]}>
      <ScrollView contentContainerStyle={{ paddingBottom: 30 }}>
        ...
        <View style={s.header}>
          ...
          <Text style={s.subtitle}>COMMAND CENTER | {totalModules} MODULES | {runningTools}/{totalTools} TOOLS</Text>
        </View>
        ...
        {/* ===== RECENT COMMANDS ===== */}
        <View style={s.section}>
          <Text style={s.sectionLabel}>◆ RECENT COMMANDS</Text>
          <View style={s.terminalBox}>
            <ScrollView ref={commandsRef} style={{ maxHeight: 180 }} nestedScrollEnabled>
              {commands.map((cmd, i) => (
                <Text key={i} style={s.terminalLine}>
                  <Text style={{ color: "#4b5563" }}>[{cmd.ts}] </Text>
                  <Text style={{ color: getCmdStatusColor(cmd.status) }}>[{cmd.status.toUpperCase()}] </Text>
                  <Text style={{ color: "#00ff88" }}>$ </Text>
                  <Text style={{ color: "#e0e7ff" }}>{cmd.cmd}</Text>
                </Text>
              ))}
            </ScrollView>
          </View>
...
const s = StyleSheet.create({
  ...
  terminalBox: { backgroundColor: "#0a0e17", borderWidth: 1, borderColor: "#1e293b", borderRadius: 8, padding: 8, borderLeftWidth: 3, borderLeftColor: "#00ff88" },
  terminalLine: { fontSize: 9, fontFamily: mono, lineHeight: 15, marginBottom: 2 },
